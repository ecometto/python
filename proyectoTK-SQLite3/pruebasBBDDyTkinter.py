...
# import sqlite3
#
# miconexion = sqlite3.connect('login.db')
# micursor = miconexion.cursor()
# micursor.execute("CREATE TABLE usuarios(Nombre TEXT,Apellido TEXT ,Usuario TEXT,Pass TEXT)")
# miconexion.commit()
# micursor.close()
# miconexion.close()
#
...
# codigo para validad datos ingresados
#
from tkinter import *
...
def nuevaVentana():  # Funcion nuevaVentana ... Nos permitira el registro de nuevos usuarios
    ...
    labelExample = tk.Label(newVentana, text="Registro : ", bg=color, font=("Arial Black", 12)).pack(
        side="top")  # Texto 'Registro'
    # panel_reg = tk.Label(newVentana, image=photo_reg).pack(
    #     side="left")  # Mostramos la imagen en la posicion 'left' (Izquierda)
    ...
    caja6.pack()
    Label(newVentana, text="Repita la Contraseña : ", bg=color,
          font=("Arial Black", 10)).pack()  # Texto 'Repita la Contraseña'
    ...
    def registro():  # Funcion registro ... Nos permitira escribir los datos a nuestra base de datos
        ...
        if (Contra_reg == Contra_reg_2):  # Esta condicion nos permite saber si las contraseñas coinciden
            ...
            c.execute(
                "INSERT INTO usuarios values(\'" + Nombre + "\',\'" + Apellido + "\',\'" + Usr_reg + "\',\'" + Contra_reg + "')")
            ...
            mb.showerror(title="Contraseña Incorrecta",
    ...
    buttons = tk.Button(newVentana, text="Registrar ¡", command=registro, bg=color,
                        font=("Arial Rounded MT Bold", 10)).pack(side="bottom")


Label(ventana, text=" ", bg=color,
      font=("Arial", 10)).pack()  # Solo es una linea vacia ... (lo use para separar el boton)
Button(text=" ENTRAR ", command=login, bg='#a6d4f2',
       font=("Arial Rounded MT Bold", 10)).pack()  # Boton ==> funcion 'login'
